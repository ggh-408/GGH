...
def sudo_solve(sudo):
    ...
    def legal_values(row, col):
        return tuple(number for number in range(1, 10) if legal(number, row, col))
        ...
        for r in range(9):
            for c in range(9):
                if not sudo[r][c]:
                    ...
                    values.append(legal_values(r, c))
        return keys, values

    # ———————————————————循环函数———————————————————
    def solve():
        coordinate = len(cache_keys) - 1
        cache_times = [0]*len(cache_keys)
