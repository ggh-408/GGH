...
def handle_focus(event):
    for dic_pair in MainWindow.Point_dic.items():
        if dic_pair[1] == event.widget and dic_pair[0][1] != 5:
    ...
    for number in range(MainWindow.Point):
        if not MainWindow.Point_dic[(number + 1, 1)].get("1.0", "end")[:-1] and \
           not MainWindow.Point_dic[(number + 1, 2)].get("1.0", "end")[:-1]:
            ...
            if not MainWindow.Point_dic[(number + 1, 3)].get("1.0", "end")[:-1]:
    ...
    for number in range(MainWindow.Point, 0, -1):
        if MainWindow.Point_dic[(number, 1)].get("1.0", "end")[:-1] or \
           MainWindow.Point_dic[(number, 2)].get("1.0", "end")[:-1]:
            ...
            MainWindow.Point_dic[(number, 3)].delete("1.0", "end")
            MainWindow.Point_dic[(number, 4)].delete("1.0", "end")
            break
...
class MainWindow(tk.Tk):
    ...
    def __init__(self):
        ...
        MainWindow.Point_dic[(0, 0)].grid(row=0, column=6)
        ...
        tk.Label(text="滚动", font=MainWindow.Font).grid(row=0, column=4)
        tk.Button(text="开始", command=self.start,    font=MainWindow.Font).grid(row=1, column=6)
        tk.Button(text="删除", command=delete_data,   font=MainWindow.Font).grid(row=2, column=6)
        tk.Button(text="清空", command=clear_data,    font=MainWindow.Font).grid(row=3, column=6)
        tk.Button(text="添加", command=self.add_data, font=MainWindow.Font).grid(row=4, column=6)
        tk.Button(text="销毁", command=destroy_data,  font=MainWindow.Font).grid(row=5, column=6)
    ...
    def add_data(self, *args):
        ...
        MainWindow.Point_dic[(MainWindow.Point, 1)].tag_add("center", "1.0", "end")
        MainWindow.Point_dic[(MainWindow.Point, 1)].tag_configure("center", justify="center")
        ...
        MainWindow.Point_dic[(MainWindow.Point, 2)].tag_add("center", "1.0", "end")
        MainWindow.Point_dic[(MainWindow.Point, 2)].tag_configure("center", justify="center")
        ...
        MainWindow.Point_dic[(MainWindow.Point, 3)].tag_add("center", "1.0", "end")
        MainWindow.Point_dic[(MainWindow.Point, 3)].tag_configure("center", justify="center")
        ...
        MainWindow.Point_dic[(MainWindow.Point, 4)].tag_add("center", "1.0", "end")
        MainWindow.Point_dic[(MainWindow.Point, 4)].tag_configure("center", justify="center")

        MainWindow.Variable.append(tk.StringVar(self))
        MainWindow.Point_dic[(MainWindow.Point, 5)] = ttk.Combobox(self, textvariable=MainWindow.Variable[-1],
                                                                   width=6, values=["左键", "右键", "移动", "滚动"],
                                                                   state="readonly", justify="center")
        MainWindow.Point_dic[(MainWindow.Point, 5)].current(0)
        MainWindow.Point_dic[(MainWindow.Point, 5)].option_add("*TCombobox*Listbox.Justify", "center")
        MainWindow.Point_dic[(MainWindow.Point, 5)].grid(row=MainWindow.Point, column=5)
    ...
    def running_operation(self):
        ...
        for number in range(MainWindow.Point):
            ...
            scroll = MainWindow.Point_dic[(number + 1, 4)].get("1.0", "end")[:-1]
            if MainWindow.Variable[number].get() != "滚动":
                if not position_x or not position_y:
                    valid_rows = number
                    break
                position_list.append((float(position_x), float(position_y)))
            else:
                position_list.append(float(scroll))
        ...
        if valid_rows >= 1:
            ...
            # wake up
            if option_list[0] == "左键":
                pyautogui.click(position_list[0])
            elif option_list[0] == "右键":
                pyautogui.rightClick(position_list[0])
            elif option_list[0] == "移动":
                pyautogui.moveTo(position_list[0])
            elif option_list[0] == "滚动":
                pyautogui.scroll(position_list[0])
            ...
            for _ in range(cycle):
                for number in range(1, valid_rows):
                    ...
                    elif option_list[number] == "滚动":
                        pyautogui.scroll(position_list[number])
